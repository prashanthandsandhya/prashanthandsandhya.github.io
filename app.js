...
(function initGallery() {
    ...
    db.ref('gallery').orderByChild('order').on('value', (snapshot) => {
        track.innerHTML = '';
        ...
        snapshot.forEach(child => {
            ...
            if (photo.type === 'video') {
                ...
                item.innerHTML = `<img src="${photo.url}" alt="${escapeHtml(photo.caption || '')}" loading="lazy">`;
            }

            if (photo.caption) {
                item.innerHTML += `<div class="gallery-caption">${escapeHtml(photo.caption)}</div>`;
            }
